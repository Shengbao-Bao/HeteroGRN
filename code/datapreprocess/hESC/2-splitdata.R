...
process_data_structured <- function(tf_file, target_file, net_file, output_prefix, dataset_type = "Specific") {
  base_dir <- "/home/bsb/mywork/data/hESC/"
  dir_map <- list(
  ...
  for (i in 1:n_fold) {
    ...
    cat(paste("Fold", i, ":\n"))
    cat(paste("  test - positive:", sum(test_set$Label == 1), "negative:", sum(test_set$Label == 0), "\n"))
    cat(paste("  val - positive:", sum(val_set$Label == 1), "negative:", sum(val_set$Label == 0), "\n"))
    cat(paste("  train - positive:", nrow(train_pos), "negative:", nrow(train_neg), "\n\n"))
