...
process_data_structured <- function(tf_file, target_file, net_file, output_prefix, dataset_type = "Specific") {
  base_dir <- "/home/bsb/mywork/data/mHSC-L/"
  dir_map <- list(
  ...
  for (i in 1:n_fold) {
    ...
    cat(paste("Fold", i, ":\n"))
    cat(paste("  test - posi:", sum(test_set$Label == 1), "nega:", sum(test_set$Label == 0), "\n"))
    cat(paste("  val - posi:", sum(val_set$Label == 1), "nega:", sum(val_set$Label == 0), "\n"))
    cat(paste("  train - posi:", nrow(train_pos), "nega:", nrow(train_neg), "\n\n"))
